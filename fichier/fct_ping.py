...
def db_ext(ip, nom, etat, latence):
	if var.db == 1:
		try:
			mysql.add_enre(ip, nom, etat, latence, var.nom_site)
		except Exception as inst:
			design.logs("ping-"+str(inst))
...
def test_ping(ip):
	try:
		...
		message = message + "\n"
		if var.db == 1:
			db_ext(ip, nom, etat, latence)
		if suivi == "X":
			fct_suivi.ecrire(ip, message)
		return message
...
def lancerping(fenetre1):
	if var.ipPing == 0:
		...
		if var.popup == 1 or var.mail == 1 or var.telegram == 1:
			threading.Thread(target=fct_thread_popup.main).start()
		if var.recap == 1:
